...
class cor06_svm():
    ...
    It uses a SVM model and the following input configuration (being rs the predicted value):
        ['tx', 'tn', 'ra', 'energyt', 'vpd', 'tdew', 'et0']

    """
    def __init__(self):
        # import model
        filename = "savedModels/SVM_Cordoba.sav"
        with open(filename, 'rb') as file:
            self.model = pickle.load(file)

        # define required inputs
        self.parameters = ['tx', 'tn', 'ra', 'energyt', 'vpd', 'tdew', 'et0']

    def import_dataset(self, fileLocation, fileType):
        """
        This function import a dataset and convert it into a pandas Dataframe
        Inputs:
            fileLocation: "data/COR06.csv"
    ...
    def getStandardDataTest(self):
        ...
        mean_list = [24.41, 11.03, 28.78, 417.59, 1.09, 10.03]
        std_list = [7.29, 3.92, 9.30, 3153.40, 4.55, 3.92]
...
if __name__ == '__main__':
    mlModel = cor06_svm()
    mlModel.import_dataset("data/COR06.csv", 'csv')
