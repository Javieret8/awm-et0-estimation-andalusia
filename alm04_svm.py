...
class alm04_svm():
    ...
    It uses a SVM model and the following input configuration (being rs the predicted value):
        ['tx', 'tn', 'ra', 'energyt', 'vpd', 'et0']

    """
    def __init__(self):
        # import model
        filename = "savedModels/SVM_Tabernas.sav"
        with open(filename, 'rb') as file:
            self.model = pickle.load(file)

        # define required inputs
        self.parameters = ['tx', 'tn', 'ra', 'energyt', 'vpd', 'et0']
    ...
    def getStandardDataTest(self):
        ...
        mean_list = [23.20, 9.84, 29.28, 392.72, 0.97]
        std_list = [5.406, 3.811, 8.845, 2524.376, 0.252]
...
if __name__ == '__main__':
    mlModel = alm04_svm()
    mlModel.import_dataset("data/ALM04.csv", 'csv')
